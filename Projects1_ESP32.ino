...
const char* ssid = "CrzyKidss"; //--> Enter your SSID / your WiFi network name.
const char* password = "12345678x"; //--> Enter your WiFi password.
/* ======================================== */

/* ======================================== Variables for telegram bot tokens. */
String BOTtoken = "6695122893:AAFjWLMnSAtJd-wgvc7NrEkJwGAcvHU7fPc";  //--> your Bot Token (Get from Botfather).
/* ======================================== */

/* ======================================== @myidbot ID */
// Use @myidbot to find out the chat ID of an individual or a group.
// Also note that you need to click "start" on a bot before it can message you.
String CHAT_ID = "5141842330";
/* ======================================== */
...
#define BUZZER_PIN  13              //--> PIR SENSOR PIN (GPIO 13)
...
byte countdown_to_stabilize_PIR_Sensor = 10;
/* ======================================== */
...
if (psramFound()) {
  ...
  config.jpeg_quality = 12;  // Meningkatkan kualitas gambar
  config.fb_count = 2;
} else {
  config.frame_size = FRAMESIZE_SVGA;
  config.jpeg_quality = 15;  // Meningkatkan kualitas gambar lebih tinggi
  config.fb_count = 1;
}
  /* ---------------------------------------- */
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start") {
      ...
      send_feedback_message += "/enable_capture_Photo_with_PIR : takes a new photo with PIR Sensor + Buzzer on\n";
      send_feedback_message += "/disable_capture_Photo_with_PIR : takes a new photo without PIR Sensor + Buzzer off\n\n";
      ...
      if(capture_Photo_with_PIR_state() == ON) {
        send_feedback_message += "- Capture Photo With PIR + BUZZER = ON\n";
      }
      if(capture_Photo_with_PIR_state() == OFF) {
        send_feedback_message += "- Capture Photo With PIR + BUZZER = OFF\n";
      }
    ...
    if (text == "/enable_capture_Photo_with_PIR") {
      ...
      send_feedback_message += "From the ESP32-CAM :\n\n";
      if(capture_Photo_with_PIR_state() == ON) {
        Serial.println("Capture Photo With PIR + BUZZER = ON");
        send_feedback_message += "Capture Photo With PIR + BUZZER = ON\n\n";
    ...
    if (text == "/disable_capture_Photo_with_PIR") {
      ...
      send_feedback_message += "From the ESP32-CAM :\n\n";
      if(capture_Photo_with_PIR_state() == OFF) {
        Serial.println("Capture Photo With PIR + BUZZER = OFF");
        send_feedback_message += "Capture Photo With PIR + BUZZER = OFF\n\n";
        botRequestDelay = 1000;
      } else {
        Serial.println("Failed to set. Try again.");
...
void setup(){
  ...
  digitalWrite(BUZZER_PIN, LOW);  // Matikan buzzer saat inisialisasi
  /* ---------------------------------------- */
...
void loop() {
 ...
 if(capture_Photo_with_PIR_state() == ON) {
    if(PIR_State() == true && PIR_Sensor_is_stable == true) {
      ...
      digitalWrite(BUZZER_PIN, LOW);      // Matikan kembali buzzer
